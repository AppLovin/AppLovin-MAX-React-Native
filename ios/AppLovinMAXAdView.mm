...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    _props = std::make_shared<const AppLovinMAXAdViewProps>();
    
    const auto &defaultProps = *std::static_pointer_cast<AppLovinMAXAdViewProps const>(_props);
    _adaptiveBannerEnabled = defaultProps.adaptiveBannerEnabled;
    _autoRefresh = defaultProps.autoRefresh;
    _loadOnMount = defaultProps.loadOnMount;
    ...
    self.onAdLoadedEvent = [self](NSDictionary *event)
    {
        if ( _eventEmitter )
        {
            ...
            AppLovinMAXAdViewEventEmitter::OnAdLoadedEvent result =
            {
                ...
                .adViewId = [[event valueForKey: @"adViewId"] intValue],
                ...
                .size = {
                    .width = [[size valueForKey: @"width"] intValue],
                    .height = [[size valueForKey: @"height"] intValue],
                }
            };
...
- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
    ...
    if ( oldViewProps.adFormat != newViewProps.adFormat )
    {
        ...
        if ( [adFormatStr al_isEqualToStringIgnoringCase: MAAdFormat.banner.label] )
        {
            _adFormat = DEVICE_SPECIFIC_ADVIEW_AD_FORMAT;
        }
        else if ( [adFormatStr al_isEqualToStringIgnoringCase: MAAdFormat.mrec.label] )
        {
    ...
    {
        _adViewId = @(newViewProps.adViewId);
    }
    ...
    {
        _adaptiveBannerEnabled = newViewProps.adaptiveBannerEnabled;
    }
    
    if ( oldViewProps.autoRefresh != newViewProps.autoRefresh )
    {
        _autoRefresh = newViewProps.autoRefresh;
    }

    if ( oldViewProps.loadOnMount != newViewProps.loadOnMount )
    {
        _loadOnMount = newViewProps.loadOnMount;
    }
